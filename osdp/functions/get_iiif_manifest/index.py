...
def handler(event, _context):
    ...
    if not row:
        return {
            "statusCode": 400,
            "body": json.dumps({"message": "No 'row' provided in event."})
        }
    ...
    if not uri:
        return {
            "statusCode": 400,
            "body": json.dumps({"message": "No 'uri' provided in row."})
        }
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({"message": "Error fetching manifest", "error": str(e)})
        }
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({"message": "Error writing to S3", "error": str(e)})
        }
    
    return {
        "statusCode": 200,
        "body": json.dumps({
            "message": "Manifest fetched and stored successfully",
            "s3_key": s3_key
        })
    }